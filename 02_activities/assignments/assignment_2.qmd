...
dataset contain 4000 samples.

```{r}
library(data.table)

# Load the family file to see sample information
fam_a2 <- fread("02_activities/data/gwa.A2.fam")
...
dataset contain 306102 SNPs.
```{r}
# Read the .bim file to count SNPs
bim_a2 <- data.table::fread("02_activities/data/gwa.A2.bim")

# Output the number of SNPs
nrow(bim_a2)
```
...
```{bash}
plink2 --bfile 02_activities/data/gwa.qc.A2_pruned \
  --king-cutoff 0.0884 \
  --out 02_activities/data/gwa_king.A2
```
...
```{bash}
plink2 --bfile 02_activities/data/gwa.qc.A2 \
  --keep 02_activities/data/gwa_king.A2.king.cutoff.in.id \
  --make-bed \
...
# Load, filter for Additive, and rename columns in one go
assoc <- fread("02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear")[TEST == "ADD"]
setnames(assoc, c("#CHROM", "POS", "ID"), c("CHR", "BP", "SNP"))

# Generate the plot
png("./02_activities/data/manhattan_plot_A2.png", width = 1000, height = 600)
manhattan(assoc, main = "Manhattan Plot: Assignment 2", col = c("blue4", "orange3"))
dev.off()
```
...
```{r}
# QQ Plot
png("./02_activities/data/qq_plot_A2.png", width = 600, height = 600)

# The qq function only needs the P-value column from your filtered data
qq(assoc$P, main = "Q-Q Plot: Assignment 2")
